...
- `db-feed` и `db-like` PostgreSQL StatefulSet
...
- `run-analysis-enabled` — включает отдельный LLM-анализ после каждого прогона, по умолчанию `true`
...
```bash
kubectl port-forward svc/argo-artifacts -n fanout 9001:9001
```

Логин и пароль берутся из Secret `argo-artifacts-credentials`:

```bash
kubectl get secret argo-artifacts-credentials -n fanout -o jsonpath='{.data.accessKey}' | base64 -d
kubectl get secret argo-artifacts-credentials -n fanout -o jsonpath='{.data.secretKey}' | base64 -d
```

`old-k6-report.html` и `new-k6-report.html` создаются встроенным k6 web dashboard export через `K6_WEB_DASHBOARD_EXPORT`.
...
- `workflow.runAnalysis.enabled` включает отдельный анализ каждого прогона
